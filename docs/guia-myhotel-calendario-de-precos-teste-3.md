---
sidebar_label: Tarifários E Disponibilidade
title: Tarifários E Disponibilidade
description: Guia detalhado sobre tarifas, disponibilidade e gerenciamento de calendário de preços no sistema hotel.
slug: /guia-myhotel-calendario-de-precos-teste-3
---

# TARIFÁRIOS E DISPONIBILIDADE

## CALENDÁRIO DE PREÇOS

### Guia Rápido

#### Página 1

TÍTULO PRINCIPAL

TARIFÁRIOS E DISPONIBILIDADE

CALENDÁRIO DE PREÇOS

#### Página 2

Neste guia, você vai aprender as funcionalidades da aba de calendário de preços.

Como chegar?  
O caminho para acessar o calendário de preços do hotel é:  
Menu de Navegação > Gestão > Tarifários e Disponibilidade > Calendário de Preços:  
Em 'Pesquisa' e os filtros disponíveis, que podem ajudar a encontrar as informações desejadas de maneira mais rápida. Veja:  
![imagem-2](/assets/guia-myhotel-calendario-de-precos-teste-3/imagem-2.webp)

#### Página 3

Consultando o Calendário de Preços:  
Após selecionar o calendário de preços que deseja consultar, é importante que você conheça as opções de pesquisa e os filtros disponíveis, que podem ajudar a encontrar as informações desejadas de maneira mais rápida. Veja:  
![imagem-3](/assets/guia-myhotel-calendario-de-precos-teste-3/imagem-3.webp)

#### Página 4

Tarifários e Pacotes:

Nesta lista, serão exibidos os tarifários e pacotes criados pelo hotel (para tarifários do tipo pacote, o sistema irá identificá-los com "[P]" antes do nome do tarifário):

Pesquisar e Agrupar por:  
Serão possíveis buscar pelo nome do ‘Tarifário’, do ‘Tipo de Quarto’, pelo nome do ‘Canal’ de vendas e pelo nome do ‘Nível de Fidelização’ (caso o hotel tenha ativada a solução BeeLoyalty):

Ordenar por:  
Será possível ordenar a lista de tarifas e pacotes por: 'Mais recentes no topo', 'Mais antigas no topo', 'Ordenar de A-Z' e 'Ordenar de Z-A':

![imagem-4](/assets/guia-myhotel-calendario-de-precos-teste-3/imagem-4.webp)  
![imagem-5](/assets/guia-myhotel-calendario-de-precos-teste-3/imagem-5.webp)

#### Página 5

Preços para:  
Escolha o número de PAX (Adultos e Crianças) de acordo com a ocupação do tipo de quarto e veja o preço cadastrado:

Mês:  
Selecione o mês que deseja consultar informações ou utilize as setas de ‘avançar ou retornar’:  
![imagem-6](/assets/guia-myhotel-calendario-de-precos-teste-3/imagem-6.webp)  
![imagem-7](/assets/guia-myhotel-calendario-de-precos-teste-3/imagem-7.webp)

#### Página 6

Visualização 1 mês:  
Permite consultar as informações do mês selecionado:  

Visualização 6 meses:  
Irá exibir um calendário que possibilita visualizar até 6 meses.  
![imagem-8](/assets/guia-myhotel-calendario-de-precos-teste-3/imagem-8.webp)  
![imagem-9](/assets/guia-myhotel-calendario-de-precos-teste-3/imagem-9.webp)

#### Página 7

Informações:

Ao estacionar o mouse neste botão, será exibida uma legenda com as cores relacionadas à disponibilidade do hotel - ‘Aberto para Vendas’, ‘Fechado para Vendas’, ‘Fechado Parcialmente para Vendas’ e ‘Allotment Indisponível’ — e que aparecerão no calendário de preços:  
Detalhes e Edição:  
![imagem-10](/assets/guia-myhotel-calendario-de-precos-teste-3/imagem-10.webp)  
![imagem-11](/assets/guia-myhotel-calendario-de-precos-teste-3/imagem-11.webp)

#### Página 8

Ao clicar em um dia, abrirá uma ‘janela de informações’, mostrando preços, disponibilidade e restrições que têm cadastrado para o tipo de quarto do tarifário:  
• Preços: valor por ‘Adulto (PAX) ou Criança’ de acordo com a ocupação configurada para o tipo de quarto.  
• Allotment: quantidade de quartos disponíveis para vendas, distribuídos por ‘Allotment’ ou ‘Ocupação’.  
• Restrições: cadastro de ‘Mín. de Dias’, ‘Máx. de Dias’, ‘Estadia Certa’, ‘Dias de Antecedência’, ‘Fechado para Chegada’, ‘Fechado para Saída’ (‘Venda Parada’: informa se tem fecho de vendas aplicado para o dia).  
E ao clicar no botão ‘Editar’ (sinalizado com ícone de lápis) no canto superior direito da janela de informações, aparecerão mais opções:  
![imagem-12](/assets/guia-myhotel-calendario-de-precos-teste-3/imagem-12.webp)

#### Página 9

Para mais informações, contate: o nosso atendimento online CHAT — ou através do e-mail:  
[servicedesk@omnibees.com](mailto:servicedesk@omnibees.com)  
![imagem-20](/assets/guia-myhotel-calendario-de-precos-teste-3/imagem-20.webp)

#### Página 10

Detalhes e Edição:  
Clique em ‘Salvar’.  
Fechar este(s) dia(s): aplicar fecho de vendas para o dia ou período selecionado.  
![imagem-13](/assets/guia-myhotel-calendario-de-precos-teste-3/imagem-13.webp)  
![imagem-14](/assets/guia-myhotel-calendario-de-precos-teste-3/imagem-14.webp)

#### Página 11

Informaçã da Tarifa:  
Informando os extras, as taxas, as ofertas e os canais que foram atribuídos ao tarifário:  
Após realizar a atualização das informações, clique em ‘Salvar’.  
Fechar este(s) dia(s): aplicar fecho de vendas para o dia ou período selecionado.  
![imagem-15](/assets/guia-myhotel-calendario-de-precos-teste-3/imagem-15.webp)  
![imagem-16](/assets/guia-myhotel-calendario-de-precos-teste-3/imagem-16.webp)

#### Página 12

Importante saber:  
Ao estacionar o mouse sobre um dia do calendário, aparecerá também o botão ‘Editar’ (sinalizado por um ícone em formato de lápis) no canto superior direito do quadrado do dia. E será possível clicar em ‘Editar’, ‘Fechar este(s) dia(s)’ e ‘Abrir este(s) dia(s)’:  
OU será possível clicar com o botão direito do mouse sobre um dia, para ter acesso ao botão ‘Editar’:  
![imagem-19](/assets/guia-myhotel-calendario-de-precos-teste-3/imagem-19.webp)

#### Página 13

Para mais informações, contate: o nosso atendimento online CHAT — ou através do e-mail:  
[servicedesk@omnibees.com](mailto:servicedesk@omnibees.com)  
![imagem-20](/assets/guia-myhotel-calendario-de-precos-teste-3/imagem-20.webp)

#### Página 14

Boas Vendas!!

#### Página 15

---

*Texto corrigido e ajustado para maior clareza e correção gramatical, mantendo a estrutura original.*